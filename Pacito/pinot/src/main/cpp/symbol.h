// $Id: symbol.h,v 1.14 2006/03/14 05:58:28 shini Exp $ -*- c++ -*-
//
...
public:
    char mark;
    SymbolSet *invokers; // contains MethodSymbols
    SymbolSet *invokees; // contains MethodSymbols
    SymbolSet *callers; // contains TypeSymbols
    ...
    bool Overrides(MethodSymbol *);
    TypeSymbol *IsVirtual();
    MethodSymbol *GetVirtual();
    virtual const wchar_t* Name() const { return name_symbol -> Name(); }
    virtual unsigned NameLength() const { return name_symbol -> NameLength(); }
    virtual const NameSymbol* Identity() const { return name_symbol; }
    const char* Utf8Name() const
...
public:
    ...
    VariableSymbol *Shadows(VariableSymbol *vsym);
    ...
    SymbolSet* subtypes; // if this->file_symbol->IsClassOnly(), then subtypes only includes the user-defined concrete classes.
                                       // for the rest, this symbolset contains all levels of subtypes
    ...
    SymbolSet* call_dependents;
    SymbolSet* instances;
    SymbolSet* references; // to be eliminated, but double check if it's a jikes var.
    SymbolSet* associates;
    ...
    bool IsFamily(TypeSymbol *type);
    bool Reifies(TypeSymbol *type)
    {
    		if (this == type)
			return true;
		else if (this -> ACC_INTERFACE())
		{
			if (type -> ACC_INTERFACE())
				return this -> IsSubinterface(type);
			else
				return false;
		}
		else
		{
			if (type -> ACC_INTERFACE())
				...
				return this -> IsSubclass(type);
		}
    }	

    bool IsOnetoMany(TypeSymbol*);
    TypeSymbol *IsOnetoMany(VariableSymbol*, DelegationTable*);
    ...
    unsigned NumMethodSymbols(); // contains construtors and methods defined within current scope only.
...
public:
    SymbolSet *aliases; // contains aliases to VariableSymbols, in one-hop
    ...
    Utf8LiteralValue* signature;
    SymbolSet *concrete_types; // TypeSymbols created with the new statement
